...
fn generate_session_id(prefix: &str, rng: &SystemRandom) -> Result<String, ring::error::Unspecified> {
    let mut bytes = [0u8; 16];
    rng.fill(&mut bytes)?;
    Ok(format!("{}-{}", prefix, hex::encode(bytes)))
}

fn generate_iv(rng: &SystemRandom) -> Result<[u8; 12], ring::error::Unspecified> {
    let mut iv = [0u8; 12];
    rng.fill(&mut iv)?;
    Ok(iv)
}

fn generate_random_hex(n: usize, rng: &SystemRandom) -> Result<String, ring::error::Unspecified> {
